%%%-------------------------------------------------------------------
%%% Created : 29 Mar 2023 by Thiago Esteves <calori@gmail.com>
%%%
...
-module(erlgame_grisp2_wbs_server).
    ...
    fun({X,Y}, #{counter := Acc, loop_map := M}) ->
      ...
      #{ counter => Acc+1, 
         loop_map => M#{ snake => Snake#{
           erlgame_grisp2_util:maybe_string_to_atom("p"++integer_to_list(Acc)) =>
           #{ x => X, y => Y}} }}
    end,
...
execute(#{<<"game">> := <<?SNAKE_SM_GAME_NAME_S>>,
  ...
  erlgame_grisp2_snake_sm:action(b2l(User), binary_to_atom(Action)),
  ok;

execute(#{<<"game">> := <<?SNAKE_SM_GAME_NAME_S>>,
          <<"user">> := User}) ->
  %% Create game and ignore if the game is already created
  case erlgame_grisp2_snake_sm_sup:create_game(b2l(User), {20,20}, 200) of
    {ok, _} -> none;
    {error, {already_started, _} } -> none
  end,
  {ok, _} = erlgame_grisp2_snake_sm:start_game(b2l(User)),
  ok;

execute(#{<<"game">> := <<?SNAKE_SM_GAME_NAME_S>>,
          <<"request">> := <<"get_best_player">> }) ->
  {ok, PlayersList} = erlgame_grisp2_db:get_best_player(?SNAKE_SM_GAME_NAME),
