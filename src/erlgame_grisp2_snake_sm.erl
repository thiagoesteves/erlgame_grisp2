...
-module(erlgame_grisp2_snake_sm).
...
start_link(UserName, Matrix, LoopTime) when is_list(UserName),
  ...
  gen_statem:start_link({local,get_registered_name(UserName)}, ?MODULE, 
    [erlgame_grisp2_util:maybe_string_to_atom(UserName), Matrix, LoopTime], []).
...
init([UserId, Matrix, LoopTime]) ->
  ...
  { NextState, GenStatemData } = case erlgame_grisp2_db:get_game_state(UserId, ?MODULE) of
    { ok, undefined } ->
      { join,
        #{ matrix      => Matrix,
           user        => UserId,
    ...
    {ok, {OldState, OldGenData} } ->
      % Clear Old State
      erlgame_grisp2_db:save_game_state(UserId, ?MODULE, undefined),
      {OldState, OldGenData}
  end,
...
terminate(Reason, State, Data = #{user := UserId}) ->
  ...
  erlgame_grisp2_db:save_game_state(UserId, ?MODULE, {State, Data}),
  ok.
...
join(enter, _OldState, GenStatemData = #{user := UserId}) ->
  ...
  {ok, Points} = erlgame_grisp2_db:get_user_points(UserId, ?MODULE),
...
start_game(UserName) when is_list(UserName) ->
  gproc:ensure_reg(?GPROC_PLAYER_GROUP(erlgame_grisp2_util:maybe_string_to_atom(UserName))),
...
update_user_actions(S = #{ matrix := {MaxX,MaxY}, user := User, points := Points, 
  ...
  %% Notify database
  erlgame_grisp2_db:add_user_points(User, ?MODULE, AddPoints),
  %% Notify web players
...
get_registered_name(UserName) ->
  erlgame_grisp2_util:maybe_string_to_atom(?MODULE_STRING ++ ":" ++ UserName).
