...
const snake_game = "erlgame_grisp2_snake_sm";
var arena = {
  row:    21,
  column: 21,
};
...
function updateSnakeBesties(json_from_erlang) {
  var obj = JSON.parse(json_from_erlang);
  // Parse Snake besties
  const besties = Object.entries(obj.best_players.erlgame_grisp2_snake_sm).map( 
    ([k,v]) =>  
...
function onMessage(evt) {
  ...
  switch (evt.data) {
    ...
    case (evt.data.match(/^(?=.*update)(?=.*erlgame_grisp2_snake_sm)/) || {}).input:
      updateSnakeBoard(evt.data);
      break;
    // Snake game best player
    case (evt.data.match(/^(?=.*best_players)(?=.*erlgame_grisp2_snake_sm)/) || {}).input:
